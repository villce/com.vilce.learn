package com.vilce.common.autoconfig.threadpool;

import com.vilce.common.model.exception.PrintExceptionInfo;
 ...
 * @EnableAsync注解的order属性设置为Ordered.LOWEST_PRECEDENCE，是为了AsyncAnnotationBeanPostProcessor的执行顺序在其它后处理器之后， 这样它就可以向现有的代理中添加代理（拦截器），而不是双重代理
 * @AutoConfigureBefore(TaskExecutionAutoConfiguration.class)注解是为了让容器中不创建默认的线程池
 * @ProjectName: com.vilce.learn
 * @Package: com.vilce.common.autoconfig.threadpool.AsyncThreadPoolAutoConfiguration
 * @Author: 雷才哲
...
@ConditionalOnProperty(prefix = "spring.vilce.async-thread-pool", name = "enable", havingValue = "true", matchIfMissing = false)
