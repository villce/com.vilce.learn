...
import com.vilce.springboot_vue.module.user.mapper.AdminRoleMenuMapper;
 ...
 * @Package: com.vilce.springboot_vue.module.user.service.impl.AdminRoleMenuServiceImpl
 * @Author: 雷才哲
    ...
    public BaseResponse updateRoleMenu(int rid, Map<String, List<Integer>> menusIds) {
        ...
        if (ObjectUtils.isNotEmpty(roleMenuList)) {
            ...
            if (!adminRoleMenuMapper.deleteRoleMenuByRid(rid)) {
                throw new BasicException(ResultStatus.ERROR.getStatus(), "删除角色菜单信息失败!");
            }
        }
        for (Integer mid : menusIds.get("menusIds")) {
            if (!adminRoleMenuMapper.addRoleMenu(rid, mid)) {
                throw new BasicException(ResultStatus.ERROR.getStatus(), "保存角色菜单信息失败!");
            }
        }
